(function() {
    ...
    function addStyles() {
        ...
        style.textContent = `
            ...
            .zerr-glow-text { animation: zerr-pulse 2s ease-in-out infinite; }
            .zerr-gradient-text {
    ...
    function createAuthModal() {
        ...
        modal.innerHTML = `
            ...
            <p style="margin:0 0 3px 0;color:#00ffcc;font-size:11px;letter-spacing:3px;font-weight:700;">âš¡ ZerrSt4rr EDITION âš¡</p>
            <p style="margin:0 0 20px 0;color:#64748b;font-size:10px;letter-spacing:1.5px;font-weight:500;">âœ¦ ENTER LICENSE KEY âœ¦</p>
            ...
            <button id="zerr-login-btn" style="width:100%;background:linear-gradient(135deg, #ff00cc, #ff0066);color:#fff;border:none;padding:12px;border-radius:10px;font-weight:800;cursor:pointer;font-size:14px;letter-spacing:1px;margin-bottom:12px;box-shadow:0 4px 15px rgba(255,0,204,0.4);transition:all 0.2s ease;text-transform:uppercase;">âœ¨ VERIFY & RUN âœ¨</button>
            <button id="zerr-telegram-btn" style="width:100%;background:linear-gradient(135deg, #0088cc, #006699);color:#fff;border:none;padding:12px;border-radius:10px;font-weight:700;cursor:pointer;font-size:14px;letter-spacing:0.5px;box-shadow:0 4px 12px rgba(0,136,204,0.3);transition:all 0.2s ease;">ðŸ“± TELEGRAM</button>
        ...
        overlay.innerHTML = `
            <div style="text-align:center;">
                <div style="position:relative; width:250px; height:250px; margin:0 auto; display:flex; align-items:center; justify-content:center;">
                    <div style="position:absolute; top:50%; left:50%; width:214px; height:214px; border-radius:50%; background:conic-gradient(transparent 0deg, #ff00cc 90deg, #ff0066 180deg, #ff00ff 270deg, transparent 360deg); filter: blur(14px); opacity:0.85; animation: zerr-fire-spin 1.5s linear infinite; z-index:1;"></div>
                    <div style="position:absolute; top:50%; left:50%; width:206px; height:206px; border-radius:50%; background:conic-gradient(transparent 0deg, #ff3300 60deg, #ff5500 120deg, #ffaa00 240deg, transparent 360deg); filter: blur(6px); opacity:0.9; animation: zerr-fire-spin 1s linear infinite reverse; z-index:2;"></div>
                    <svg width="240" height="240" style="transform:rotate(-90deg); position:relative; z-index:3;">
                        <circle cx="120" cy="120" r="95" fill="rgba(6,10,23,0.65)" stroke="rgba(255,0,204,0.2)" stroke-width="14"></circle>
                        <circle id="zerr-progress" cx="120" cy="120" r="95" fill="none" stroke="#ff00cc" stroke-width="14" stroke-dasharray="${circumference}" stroke-dashoffset="${circumference}" stroke-linecap="round" style="filter: drop-shadow(0 0 6px #ff00cc); transition: stroke-dashoffset 1s linear;"></circle>
                    </svg>
                    <div id="zerr-countdown-text" style="position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); font-size:54px; font-weight:900; color:#fff; text-shadow:0 0 20px #ff00cc, 0 0 30px rgba(255,0,204,0.3); z-index:4;">${countdownStart}</div>
                </div>
                <p style="margin-top:30px; color:#ff00cc; font-size:16px; font-weight:700; letter-spacing:3px; text-shadow:0 0 12px rgba(255,0,204,0.4); position:relative; z-index:4;">âš¡ REDIRECTING TO ZERR âš¡</p>
            </div>
        `;
    ...
    async function showUpdateOverlay() {
        return new Promise(async (resolve) => {
            ...
            updateOverlay.innerHTML = `
                <div style="text-align:center; background:rgba(6,10,23,0.98); padding:35px 30px; border-radius:20px; border:2px solid #ff00cc; width:300px; animation: zerr-lightning-glow 3s linear infinite; box-shadow:0 0 30px rgba(255,0,204,0.3);">
                    <div style="width:50px; height:50px; border:4px solid rgba(255,0,204,0.1); border-top:4px solid #ff00cc; border-radius:50%; margin:0 auto 20px auto; animation: zerr-spin 0.8s linear infinite; box-shadow:0 0 15px rgba(255,0,204,0.3);"></div>
                    <p id="zerr-check-text" style="color:#ff00cc; font-size:15px; font-weight:700; margin:0; letter-spacing:1.5px; text-shadow:0 0 8px rgba(255,0,204,0.3);" class="zerr-glow-text">CHECKING UPDATE...</p>
                </div>
            `;
            ...
            if (checkText) {
                if (updateAvailable) {
                    checkText.innerHTML = "<span style='color:#00ffcc;'>âœ“ LINK UPDATED SUCCESSFULLY! âœ“</span>";
                } else {
                    checkText.innerHTML = "<span style='color:#ff6666; text-shadow:0 0 8px rgba(255,102,102,0.3);'>âœ— NO UPDATE AVAILABLE âœ—</span>";
                }
            }
    ...
    async function handleSuccessfulLogin(modal, statusElement, loginBtn, telegramBtn) {
        statusElement.innerHTML = "<span style='color:#00ffcc; text-shadow:0 0 5px #00ffcc;'>âœ“ KEY VALIDATED! WELCOME ZERRST4RR! âœ“</span>";
        setTimeout(async () => {
            modal.remove();
    ...
    function setupLoginButton(modal, statusElement, loginBtn, telegramBtn, inputField) {
        loginBtn.addEventListener("click", async () => {
            ...
            if (!inputKey) {
                statusElement.innerHTML = "<span style='color:#ff6666;'>âš ï¸ PLEASE INPUT KEY! âš ï¸</span>";
                return;
            }
            statusElement.innerHTML = "<span style='color:#ff00cc; text-shadow:0 0 8px rgba(255,0,204,0.5);'>ðŸ”— CONNECTING TO ZERR SERVER...</span>";
            ...
            try {
                ...
                if (validKeys.includes(inputKey)) {
                    ...
                    statusElement.innerHTML = "<span style='color:#ff6666;'>âŒ INVALID LICENSE KEY! âŒ</span>";
                    loginBtn.disabled = false;
                    telegramBtn.disabled = false;
                }
            } catch (e) {
                statusElement.innerHTML = "<span style='color:#ff6666;'>âš ï¸ SERVER ERROR! TRY AGAIN âš ï¸</span>";
                loginBtn.disabled = false;
                telegramBtn.disabled = false;
            }
        });
